...
export function ProjectsSection() {
  return (
    <section
      id="projects"
      className="relative z-10 -mt-10 overflow-x-clip overflow-y-visible rounded-t-[40px] section-deep text-portfolio-ink sm:-mt-12 sm:rounded-t-[50px] md:-mt-14 md:rounded-t-[60px]"
    >
      <div className="px-5 pt-20 pb-[max(8rem,calc(env(safe-area-inset-bottom)+6rem))] max-sm:pb-[max(15rem,calc(env(safe-area-inset-bottom)+9.5rem))] sm:px-8 sm:pb-[max(9rem,calc(env(safe-area-inset-bottom)+6.5rem))] md:px-10 md:pb-[max(10rem,calc(env(safe-area-inset-bottom)+7rem))]">
        ...
        <p className="mx-auto mb-10 max-w-2xl px-1 text-center text-xs font-medium uppercase leading-relaxed tracking-[0.18em] text-portfolio-muted sm:mb-14 sm:px-0 sm:text-[13px] sm:tracking-[0.22em] md:mb-16">
        ...
        <div className="max-sm:-mx-5 max-sm:w-[calc(100%+2.5rem)] sm:mx-0 sm:w-full max-sm:pb-6 sm:pb-8">
          <ProjectsCarousel3D />
        </div>
      </div>
    </section>
