  ...
  return (
    <section
      ...
      style={{
        ...
        backgroundImage: [
          /* Continuation of About’s indigo wash, meeting Skills headline energy */
          'radial-gradient(ellipse 130% 90% at 50% -12%, rgba(99, 102, 241, 0.1) 0%, transparent 52%)',
          'radial-gradient(ellipse 85% 55% at 12% 0%, rgba(99, 102, 241, 0.06) 0%, transparent 48%)',
          'radial-gradient(ellipse 90% 60% at 88% 4%, rgba(67, 56, 202, 0.08) 0%, transparent 50%)',
        ].join(', '),
      }}
    >
      <div className="relative z-[2] -mt-1 sm:-mt-2 md:-mt-3">
        <div
          className="relative overflow-hidden rounded-b-[32px] pb-14 pt-8 pl-[max(1rem,env(safe-area-inset-left))] pr-[max(1rem,env(safe-area-inset-right))] shadow-[0_32px_80px_-28px_rgba(15,23,42,0.18)] max-sm:rounded-b-[28px] max-sm:pb-12 max-sm:pt-6 sm:rounded-b-[40px] sm:pb-16 sm:pl-[max(2rem,env(safe-area-inset-left))] sm:pr-[max(2rem,env(safe-area-inset-right))] sm:pt-10 md:pb-20 md:pl-[max(2.5rem,env(safe-area-inset-left))] md:pr-[max(2.5rem,env(safe-area-inset-right))] md:pt-12"
          style={{
            backgroundImage: [
              'radial-gradient(ellipse 125% 75% at 50% -5%, rgba(99,102,241,0.07) 0%, transparent 52%)',
              'radial-gradient(ellipse 90% 60% at 88% 8%, rgba(67,56,202,0.06) 0%, transparent 48%)',
            ].join(', '),
            backgroundColor: '#f1f5f9',
          }}
        >
          <h2
            className="relative z-10 mb-4 px-3 bg-gradient-to-b from-slate-900 via-slate-700 to-slate-400 bg-clip-text text-center font-black uppercase text-transparent max-sm:break-words sm:mb-6 sm:px-4"
            style={{ fontSize: 'clamp(3rem, 12vw, 160px)', wordBreak: 'break-word', hyphens: 'auto' }}
          >
            Skills
          </h2>
          <p
            className="relative z-10 mx-auto mb-12 max-w-2xl px-4 text-center font-medium leading-relaxed text-slate-500 max-sm:mb-10 sm:mb-16 sm:px-6 md:mb-20"
      ...
      <div
        ...
        style={{
          backgroundImage: [
            'linear-gradient(180deg, rgba(241, 245, 249, 0) 0%, rgba(241, 245, 249, 0.6) 52%, #f2f1ef 100%)',
            'radial-gradient(ellipse 110% 85% at 50% 0%, rgba(99, 102, 241, 0.1) 0%, transparent 58%)',
            'radial-gradient(ellipse 75% 50% at 88% 22%, rgba(67, 56, 202, 0.08) 0%, transparent 50%)',
          ].join(', '),
        }}
      />
      ...
      <div
        ...
        style={{
          ...
          backgroundImage: [
            'radial-gradient(ellipse 130% 70% at 50% -8%, rgba(99, 102, 241, 0.06) 0%, transparent 48%)',
            'radial-gradient(ellipse 80% 45% at 10% 0%, rgba(67, 56, 202, 0.05) 0%, transparent 45%)',
          ].join(', '),
        }}
      >
        <div
          ...
          <div className="flex flex-col gap-10">
            {EXPERIENCE.map((job, i) => (
              <FadeIn key={`${job.org}-${job.period}`} delay={i * 0.08} duration={0.65} y={24}>
                <div className="border-l-[3px] border-indigo-500 pl-6 sm:pl-8">
                  ...
                  <p className="mt-1 text-sm font-semibold text-indigo-600">{job.org}</p>
