...
/** prefers-reduced-motion — local hook so this file stays free of framer-motion. */
...
function createVortex(
  ...
  function build() {
    ...
    cometTex = track(
      blob(32, [
        ...
        [0.3, 'rgba(72,229,194,0.45)'],
        [0.7, 'rgba(72,229,194,0.08)'],
        [1, 'rgba(0,0,0,0)'],
      ]),
    );
    ...
    for (let i = 0; i < cometTotal; i++) {
      ...
      cometList.push({
        ...
        s: 0,
        idle: 0,
        idleFor: 0.4 + (i / Math.max(1, cometTotal)) * cfg.cometDelay,
        trail,
        trailCol,
        geo,
        line,
  ...
  return {
    ...
    dispose() {
      ...
      for (const d of disposables) d.dispose();
      // Do not forceContextLoss — it poisons the canvas for HMR remounts.
      renderer.dispose();
    },
  };
}

export type HeroVortexProps = {
  className?: string;
  /** Soften the effect so hero copy stays readable. */
  opacity?: number;
};

/**
 * Particle tornado behind the hero — strands, flickering dots, teal comets.
 * Pauses off-screen and respects prefers-reduced-motion.
 */
export function HeroVortex({ className = '', opacity = 0.55 }: HeroVortexProps) {
  ...
  const line = {
    ...DEFAULTS.lineOptions,
    count: isNarrow ? 90 : DEFAULTS.lineOptions.count,
  };
  const dot = {
    ...DEFAULTS.dotOptions,
    count: isNarrow ? 2200 : DEFAULTS.dotOptions.count,
  };
  const comet = {
    ...DEFAULTS.cometOptions,
    count: isNarrow ? 4 : DEFAULTS.cometOptions.count,
  };

  const config: VortexConfig = {
    floorRadius: DEFAULTS.bottomRadius / PX_PER_WORLD,
    waistRadius: DEFAULTS.waistRadius / PX_PER_WORLD,
    crownRadius: DEFAULTS.topRadius / PX_PER_WORLD,
    waistAt: 1 - DEFAULTS.waistPosition / 100,
    twist: DEFAULTS.twist,
    zoom: DEFAULTS.zoom,
    flowDir: DEFAULTS.direction === 'left' ? -1 : 1,
    flowSpeed: (DEFAULTS.speed / 100) * (DEFAULTS.direction === 'left' ? -1 : 1),
    lineCount: line.count,
    lineColor: line.color,
    lineGlow: (line.glow / 10) * LINE_GLOW_MAX,
    showDots: DEFAULTS.dots,
    dotCount: dot.count,
    dotSize: dot.size / DOT_SIZE_SCALE,
    dotColor: dot.color,
    dotGlow: (dot.glow / 10) * DOT_GLOW_MAX,
    dotFlicker: dot.flicker / 10,
    showComets: DEFAULTS.comets,
    ...
    hoverRepel: DEFAULTS.repel,
    repelRadius: DEFAULTS.repelOptions.radius,
    repelStrength: DEFAULTS.repelOptions.strength,
    running,
  };
  ...
  // Fresh <canvas> every mount — avoids poisoned WebGL contexts after HMR dispose.
  useEffect(() => {
    ...
    try {
      ...
      console.warn('[HeroVortex] init failed:', err);
      canvas.remove();
      return;
    }
